...
- 논리적 정합성 문제나 의존성, 전반적인 https 규격을 다시 한번 체크하는 등 다양한 시도를 진행했습니다. 알고보니 결국 Tmap 지도 객체에 https 호출에 응답할 수 있는 변수를 세팅해주지 않은것이 원인인 단순 syntax 에러였던 것으로 확인되어 간단히 해결했습니다.

**Socket & Redux**
- 소켓을 redux middleware를 활용해 컴포넌트 렌더링 로직과 분리 -> 명확한 관심사의 분리를 통해 향후 소켓로직 작업시 혼동 최소화 및 작업속도 향상을 이뤘습니다. 
